...
import {
  Base6PassportCover,
  Base6PassportIdentityPage,
} from "@/components/base6/PassportDesign";
import { OneBoardingPass } from "@/components/onboarding/OneBoardingPass";
import { getBase6BoardingAssignment } from "@/lib/boarding";
import {
  queuePassportStampAwardsForLounge,
  type PassportStampAwardDetail,
} from "@/lib/passportStampEvents";
import {
  createBoardingPassDetails,
  createPassportIdentityDetails,
...
export default function SignupPage() {
  ...
  const [boardingAssignment, setBoardingAssignment] = useState(() =>
    getBase6BoardingAssignment(1),
  );
  const [earnedStampAwards, setEarnedStampAwards] = useState<
    PassportStampAwardDetail[]
  >([]);
  ...
  const dynamicTitle =
    step.key === "platform" && form.username.trim()
  ...
  const businessReason = getBusinessReason(form);
  const currentYear = new Date().getFullYear();
  const sanAndreasYears = Array.from(
    { length: currentYear - 2013 + 1 },
    (_, index) => String(2013 + index),
  );
  ...
  const canContinue =
    step.key === "business"
      ...
      : step.key === "crimeHistory"
        ? form.crimeHistory.trim().length > 0 &&
          (!declaredSanAndreas || form.sanAndreasYear.trim().length > 0)
        : step.key === "email"
  ...
  const passportIssueDate = useMemo(
    () =>
  ...
  const passportNumber = useMemo(
    () => createPassportNumber(form.username, form.platform),
  ...
  function updateProfileImage(file: File | null) {
    ...
    const finishScan = () => {
      ...
      window.setTimeout(
        ...
        Math.max(minimumScanTime - elapsed, 0),
      );
    };
  ...
  async function createBase6Account() {
    ...
    if (!hasSupabaseEnv || !supabase) {
      setAuthError(
        "Add your Supabase URL and anon key to .env.local before creating real passports.",
      );
      return false;
    }
    ...
    try {
      ...
      if (!user) {
        throw new Error(
          "Supabase did not return a user. Check your auth provider settings.",
        );
      }
      ...
      const { data: profileData, error: profileError } = await supabase
        ...
        .insert({
          ...
          san_andreas_since_year: declaredSanAndreas
            ? Number(form.sanAndreasYear || "2013")
            : null,
        ...
        .select(
          "boarding_sequence, boarding_group, boarding_flight, boarding_seat",
        )
        .single();
      ...
      setBoardingAssignment({
        ...
        boardingGroup:
          profileData?.boarding_group || fallbackAssignment.boardingGroup,
        boardingFlight:
          profileData?.boarding_flight || fallbackAssignment.boardingFlight,
        boardingSeat:
          profileData?.boarding_seat || fallbackAssignment.boardingSeat,
      });

      const stampCodes = declaredSanAndreas
        ? ["checked_in", "san_andreas_veteran"]
        : ["checked_in"];
      ...
      if (stampRows?.length) {
        ...
        const awardedStamps = stampRows
          .sort(
            (a, b) => stampCodes.indexOf(a.code) - stampCodes.indexOf(b.code),
          )
          .map((stamp) => ({
            ...
            icon: stamp.icon,
            description:
              stamp.description || "Stamped into your Leonida Passport.",
  ...
  const boardingPassDetails = useMemo(
    () =>
      createBoardingPassDetails({
        form,
        passportNumber,
    ...
    [arrivalStamp, boardingAssignment, form, passportNumber],
  );

  const passportIdentityDetails = useMemo(
    () =>
      createPassportIdentityDetails({
        form,
        passportNumber,
  ...
  async function createBoardingPassShareImage() {
    ...
    headerGradient.addColorStop(0.48, "#cc48f0");
    ...
    let cursor = bodyX;
    const barSteps = [
      4, 3, 7, 4, 2, 6, 3, 5, 2, 8, 4, 3, 6, 2, 5, 4, 7, 3, 2, 6, 4, 5, 3, 8,
    ];
    ...
    drawCanvasText(
      context,
      `Passport no. ${boardingPassDetails.passportNumber}`,
      mainX,
      121,
      mainW,
    );
    ...
    context.textAlign = "center";
    context.fillText(
      "PLEASE BE AT THE GATE WHEN BOARDING BEGINS",
      passWidth / 2,
      263,
    );
    context.textAlign = "left";
  ...
  async function shareBoardingPass() {
    ...
    try {
      ...
      const file = new File(
        [blob],
        `base6-boarding-pass-${passportNumber}.png`,
        { type: "image/png" },
      );
    ...
    return await new Promise<string>((resolve, reject) => {
      ...
      reader.onload = () =>
        typeof reader.result === "string"
          ...
          : reject(new Error("Could not read boarding pass image."));
      reader.onerror = () =>
        reject(new Error("Could not read boarding pass image."));
  ...
  async function loadImageFromDataUrl(dataUrl: string) {
    return await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      image.onerror = () =>
        reject(new Error("Could not prepare boarding pass for PDF."));
      image.src = dataUrl;
    });
  }
  ...
  function createPdfFromJpegBytes(
  ...
  async function downloadBoardingPassPdf() {
    ...
    try {
      ...
      const { bytes, width, height } =
        await convertPngBlobToJpegBytes(imageBlob);
      ...
      setShareStatus(
        error instanceof Error
          ? error.message
          : "Could not create printable PDF.",
      );
    }
  }
  ...
  return (
    <div className={`signup-screen signup-phase-${phase}`}>
      ...
      {(phase === "details" || phase === "fading") && (
        <section className="boarding-shell signup-details-shell">
          <div className="boarding-topline">
            <Link className="boarding-back-link" href="/">
              ← Back to gate
            </Link>
          ...
          <div key={transitionKey} className="boarding-step-card">
            ...
            <div className="boarding-answer">
              {step.type === "text" && (
                <input
                  ...
                  value={value}
                  onChange={(event) =>
                    updateField(step.key, event.target.value)
                  }
                  onKeyDown={(event) => {
                ...
                <div className="boarding-photo-upload">
                  ...
                  <label className="boarding-upload-button">
                    <input
                      ...
                      accept="image/*"
                      onChange={(event) =>
                        updateProfileImage(event.target.files?.[0] ?? null)
                      }
                    />
                  ...
                  <div className="boarding-photo-actions">
                    <button
                      className="boarding-skip-button"
                      type="button"
                      onClick={nextStep}
                    >
                  ...
                  <p className="boarding-upload-note">
                    You can skip this for now and add a passport photo later.
                  </p>
                </div>
              )}
                ...
                <div className="boarding-declaration-stack">
                    ...
                    {step.options.map((option) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (
                            option === "Spent time in San Andreas" &&
                            !form.sanAndreasYear
                          ) {
                  ...
                  {declaredSanAndreas && (
                    <div className="boarding-sub-question">
                      <label htmlFor="san-andreas-year">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          updateField("sanAndreasYear", event.target.value)
                        }
                      >
                      ...
                      <p>
                        Starting from 2013 — the year San Andreas opened for GTA
                        V.
                      </p>
                    </div>
                  )}
              ...
              {step.type === "auth" && (
                <div className="boarding-auth-stack">
                  ...
                  />
                  <p className="boarding-upload-note">
                    For local testing, turn off email confirmation in Supabase
                    Auth so profiles can save immediately.
                  </p>
                </div>
              )}
            ...
            {authError && (
              <p className="boarding-error" role="alert">
                {authError}
              </p>
            )}
            ...
            <div className="boarding-actions">
              <button
                ...
                disabled={stepIndex === 0 || phase !== "details"}
              >
                Back
              </button>
              {!isLastStep ? (
                <button
                  ...
                  type="button"
                  onClick={nextStep}
                  disabled={!canContinue || phase !== "details"}
                >
                  Continue
                </button>
              ) : (
                <button
                  className="button primary"
                  type="button"
                  onClick={printBoardingPass}
                  disabled={
                    !canContinue || phase !== "details" || isCreatingAccount
                  }
                >
                  {isCreatingAccount
                    ? "Issuing passport..."
      ...
      {phase === "assigned" && (
        <section
          className="boarding-shell seat-assignment-shell"
          aria-label="Assigned seat and flight"
        >
          <div className="boarding-topline">
          ...
          <div className="seat-assignment-card">
            ...
            <h1 className="h1 boarding-title">
              Your place on the flight to Leonida is confirmed.
            </h1>
            <p className="copy boarding-copy">
              Keep this with your passport. Your boarding pass will print next.
            </p>
            ...
            <div className="seat-assignment-meta">
              ...
              <small>
                Boarding group {boardingAssignment.boardingGroup} · Destination
                Leonida
              </small>
            </div>
          </div>
      ...
      {(phase === "ticket" || phase === "showcase" || phase === "leaving") && (
        <section
          className="ticket-print-stage base6-ticket-stage"
          aria-label="Printed boarding pass"
        >
          {phase === "ticket" && (
            <>
              ...
              >
                <div
                  className="printed-document__fixed-feed"
                  aria-hidden="true"
                >
                  <span className="printed-document__feed-label">
                    BASE6 printer feed
                  </span>
                </div>
                <div className="printed-document__tear" aria-hidden="true">
                  <span className="printed-document__tear-pulse" />
                  <span className="printed-document__tear-copy">
                    Swipe across tear line
                  </span>
                </div>
                <div className="printed-document__pass">
                  <span
                    className="printed-document__ripped-edge"
                    aria-hidden="true"
                  />
                  <OneBoardingPass details={boardingPassDetails} />
                </div>
              </div>
          ...
          {phase === "showcase" && (
            <section
              className={`passport-reveal-scene ${ticketReady ? "ready" : ""} ${passportOpen ? "passport-is-open" : ""}`}
              aria-label="Leonida passport reveal"
            >
              {!passportOpen ? (
                <>
                  <div
                    className="passport-ticket-pocket"
                    aria-hidden={!ticketReady}
                  >
                  ...
                  <button
                    className="button primary open-passport-cta"
                    type="button"
                ...
                <>
                  <div className="open-passport-layout">
                    <div
                      className="base6-open-passport-spread"
                      aria-label="First pages of Leonida passport"
                    >
                      <Base6PassportIdentityPage
                        details={passportIdentityDetails}
                      />
                    </div>
                  ...
                  <div className="boarding-pass-share-panel">
                    <div className="boarding-pass-action-row">
                      <button
                        className="button secondary share-boarding-pass-cta"
                        type="button"
                      ...
                      <button
                        className="button secondary print-boarding-pass-cta"
                        type="button"
                        onClick={downloadBoardingPassPdf}
                      >
                        Print Boarding Pass
                      </button>
                    </div>
                    <p>
                      You can share or print this later from your documents
                      page.
                    </p>
                    {shareStatus && <span>{shareStatus}</span>}
                  </div>
                  <button
                    className="button primary enter-lounge-cta"
                    type="button"
                    onClick={() => {
                      queuePassportStampAwardsForLounge(earnedStampAwards);
                      setPhase("leaving");
      ...
      {isScanningId && (
        <div className="id-scan-screen" role="status" aria-live="polite">
          <div className="id-scan-card">
            ...
            <div className="id-scan-loader" aria-hidden="true">
              <span />
            </div>
          </div>
        </div>
