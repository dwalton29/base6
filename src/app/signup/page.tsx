...
export default function SignupPage() {
  ...
  function moveTear(event: PointerEvent<HTMLDivElement>) {
    ...
    if (nextProgress >= 92) {
      ...
      setPhase("showcase");
      window.setTimeout(() => setTicketReady(true), 1450);
    }
  }
  ...
  return (
    <div className={`signup-screen signup-phase-${phase}`}>
      ...
      {(phase === "ticket" || phase === "showcase" || phase === "leaving") && (
        <section className="ticket-print-stage" aria-label="Printed boarding pass">
          <div className="printer-slot" aria-hidden="true">
            <span />
          </div>
          <div
            className={`boarding-pass-ticket printed-ticket ${isTearing ? "is-tearing" : ""} ${phase === "showcase" ? "ticket-showcase-mode" : ""}`}
          ...
          >
            <div className="printed-ticket-rotated">
              <div className="ticket-main">
                <div className="ticket-passenger-row">
                  <div className="ticket-photo-id" aria-label="Passenger photo ID">
                    {form.profileImage ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={form.profileImage} alt="Passenger ID" />
                    ) : (
                      <span>{form.username ? form.username.slice(0, 2).toUpperCase() : "ID"}</span>
                    )}
                  </div>
                  <div className="ticket-passenger-copy">
                    <span className="eyebrow">BASE6 passport control</span>
                    <h1>Leonida Boarding Pass</h1>
                    <p>Passenger checked in. Passport approved for entry to Leonida.</p>
                  </div>
                </div>
                <div className="ticket-details">
                  <span><strong>Name</strong>{form.username || "Passenger"}</span>
                  <span><strong>Platform</strong>{form.platform || "Pending"}</span>
                  <span><strong>{platformLabel}</strong>{form.handle || "Pending"}</span>
                  <span><strong>Record</strong>{recordLabel || "Clean"}</span>
                  <span><strong>Business</strong>{businessReason || "Lounge"}</span>
                  <span><strong>Passport no.</strong>{passportNumber}</span>
                </div>
              </div>
            ...
            {phase === "ticket" && (
              <div className="tear-line" aria-hidden="true">
                <span className="tear-fill" />
                <span className="tear-handle">Swipe to tear →</span>
              </div>
            )}
          </div>

          {phase === "showcase" && (
            <div className={`ticket-showcase-copy ${ticketReady ? "ready" : ""}`}>
              <span className="eyebrow">Passport approved</span>
              <h2>Welcome to Leonida.</h2>
              <p>Your boarding pass has been issued. Keep it clean — your travel record follows you.</p>
              <div className="ticket-showcase-actions">
                ...
                <button className="button share-boarding-pass" type="button" onClick={shareBoardingPass} disabled={!ticketReady}>
                  Share boarding pass
                </button>
              </div>
            </div>
